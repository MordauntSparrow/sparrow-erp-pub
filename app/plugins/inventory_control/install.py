...
def _ensure_med_bag_schema(conn) -> None:
    ...
    INV-KIT-002 baseline, CURA-DRUG-INV-001 ledger).
    """
    _create_table(
        conn,
        "inventory_med_bag_templates",
        """
        id INT AUTO_INCREMENT PRIMARY KEY,
        name VARCHAR(255) NOT NULL,
        code VARCHAR(64) NOT NULL,
        bag_kind VARCHAR(48) NOT NULL DEFAULT 'general',
        description TEXT NULL,
        is_active TINYINT(1) NOT NULL DEFAULT 1,
        created_at TIMESTAMP NOT NULL DEFAULT CURRENT_TIMESTAMP,
        updated_at TIMESTAMP NOT NULL DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP,
        UNIQUE KEY uniq_med_bag_tpl_code (code),
        KEY idx_med_bag_tpl_kind (bag_kind),
        KEY idx_med_bag_tpl_active (is_active)
        """,
    )
    _create_table(
        conn,
        "inventory_med_bag_template_lines",
        """
        id INT AUTO_INCREMENT PRIMARY KEY,
        template_id INT NOT NULL,
        inventory_item_id INT NOT NULL,
        expected_qty DECIMAL(18, 4) NOT NULL DEFAULT 1,
        sort_order INT NOT NULL DEFAULT 0,
        notes VARCHAR(512) NULL,
        KEY idx_med_bag_tl_tpl (template_id),
        KEY idx_med_bag_tl_item (inventory_item_id)
        """,
    )
    _create_table(
        conn,
        ...
        template_id INT NOT NULL,
        public_asset_number VARCHAR(64) NULL,
        status VARCHAR(24) NOT NULL DEFAULT 'in_store',
        assignee_type VARCHAR(24) NULL,
        assignee_id VARCHAR(128) NULL,
        equipment_asset_id BIGINT NULL,
        parent_instance_id BIGINT NULL,
        tamper_seal_id VARCHAR(128) NULL,
        tamper_seal_colour VARCHAR(16) NULL,
        ...
        created_at TIMESTAMP NOT NULL DEFAULT CURRENT_TIMESTAMP,
        updated_at TIMESTAMP NOT NULL DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP,
        UNIQUE KEY uniq_med_bag_pub (public_asset_number),
        KEY idx_med_bag_inst_tpl (template_id),
        KEY idx_med_bag_inst_status (status),
        KEY idx_med_bag_inst_parent (parent_instance_id)
        """,
    )
    _create_table(
        conn,
        "inventory_med_bag_instance_lines",
        """
        id BIGINT AUTO_INCREMENT PRIMARY KEY,
        instance_id BIGINT NOT NULL,
        inventory_item_id INT NOT NULL,
        quantity_expected DECIMAL(18, 4) NOT NULL DEFAULT 0,
        quantity_on_bag DECIMAL(18, 4) NOT NULL DEFAULT 0,
        batch_id INT NULL,
        lot_number VARCHAR(128) NULL,
        expiry_date DATE NULL,
        return_status VARCHAR(24) NULL,
        KEY idx_med_bag_il_inst (instance_id),
        KEY idx_med_bag_il_item (inventory_item_id),
        KEY idx_med_bag_il_lot (lot_number)
        """,
    )
    _create_table(
        conn,
        "inventory_med_bag_ledger",
        """
        id BIGINT AUTO_INCREMENT PRIMARY KEY,
        instance_id BIGINT NOT NULL,
        ...
        batch_id INT NULL,
        epcr_external_ref VARCHAR(128) NULL,
        epcr_episode_ref VARCHAR(128) NULL,
        idempotency_key VARCHAR(128) NULL,
        lot_number VARCHAR(128) NULL,
        witness_user_id VARCHAR(64) NULL,
        witness_user_id_2 VARCHAR(64) NULL,
        performed_by VARCHAR(128) NULL,
        notes TEXT NULL,
        created_at TIMESTAMP NOT NULL DEFAULT CURRENT_TIMESTAMP,
        UNIQUE KEY uniq_med_bag_idem (idempotency_key),
        KEY idx_med_bag_led_inst (instance_id, created_at),
        KEY idx_med_bag_led_epcr (epcr_external_ref),
        KEY idx_med_bag_led_lot (lot_number)
        """,
    )
    _create_table(
        conn,
        "inventory_med_witness_rules",
        """
        action_type VARCHAR(64) NOT NULL PRIMARY KEY,
        witness_count TINYINT NOT NULL DEFAULT 0,
    ...
    _create_table(
        conn,
        "inventory_med_bag_seal_events",
        """
        id BIGINT AUTO_INCREMENT PRIMARY KEY,
        instance_id BIGINT NOT NULL,
        event_type VARCHAR(32) NOT NULL,
        expected_seal_id VARCHAR(128) NULL,
        expected_colour VARCHAR(16) NULL,
        expected_initial VARCHAR(16) NULL,
        entered_seal_id VARCHAR(128) NULL,
        entered_colour VARCHAR(16) NULL,
    ...
    _try_add_med_bag_parent_fk(conn)

    for col in (
        "tamper_seal_id VARCHAR(128) NULL",
        "tamper_seal_colour VARCHAR(64) NULL",
        "tamper_seal_set_at TIMESTAMP NULL",
        "tamper_seal_set_by VARCHAR(128) NULL",
        "tamper_seal_initial VARCHAR(16) NULL",
...
def _try_add_med_bag_parent_fk(conn) -> None:
    """Self-FK: pod / nested bag contained in a parent bag instance (idempotent)."""
    cur = conn.cursor()
    try:
        cur.execute(
            """
            ALTER TABLE inventory_med_bag_instances
            ADD CONSTRAINT fk_med_bag_parent_instance
            FOREIGN KEY (parent_instance_id) REFERENCES inventory_med_bag_instances(id)
            ON DELETE RESTRICT
            """
        )
        conn.commit()
        print("[inventory_control] Added FK fk_med_bag_parent_instance")
    except Exception as e:
        try:
            conn.rollback()
        except Exception:
        ...
        if (
            ...
            or "errno 1826" in msg
            or "errno 121" in msg
        ):
            pass
        else:
            print(f"[inventory_control] FK fk_med_bag_parent_instance: {e}")
    finally:
        try:
            cur.close()
        except Exception:
