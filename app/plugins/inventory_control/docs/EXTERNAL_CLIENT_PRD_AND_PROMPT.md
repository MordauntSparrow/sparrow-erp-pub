# Inventory Control — external client PRD & build guide

This document is a **Product Requirements Document (PRD)** and implementation guide for custom frontends (mobile, MDT, integrations) and the in-browser admin UI. External API clients should use **`Authorization: Bearer`** (JWT from core `POST /api/login`) where supported; same-origin admin pages use **session cookies + CSRF** (`X-CSRFToken`). Use it as a single source of truth for scope, API, settings, and auth.
...
- **All API requests:**  
  - Same-origin admin: send **cookies** with every request and **X-CSRFToken** for unsafe methods. Cross-origin API clients: use **Bearer JWT** (no cookies).  
...
Base path is **configurable** (see Settings). Below, `{base}` = that base (e.g. `/plugin/inventory_control`). Browser admin endpoints use **session auth** (cookies + CSRF for non-GET). **JWT Bearer** is for mobile/integrations and supplier-facing endpoints where documented.
...
Supplier-only endpoints (Bearer token) are omitted here; the main browser admin UI uses session auth.
...
## 7. Build by component (implementation prompts)
...
## 9. Prompt closer (for AI-assisted builds)
...
Use a planning step when you want an AI builder to ask clarifying questions before implementing.
...
This document is the single source for features, endpoints, settings, and auth for Inventory Control external clients and admin UI.
