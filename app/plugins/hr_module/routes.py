...
def admin_employee_new():
    ...
    if request.method == "POST":
        role_new = (request.form.get("role_name_new") or "").strip().lower()
        role_pick = (request.form.get("role_name") or "").strip().lower()
        role_name = role_new or role_pick or None
    ...
    return render_template(
        ...
        roles=roles,
        default_tb_role=hr_services.default_employee_tb_role_name(),
        config=_core_manifest,
    )
...
def admin_contractor_profile(cid):
    ...
    try:
        ...
        pass
    dbs_logs = []
    if is_dbs_update_service_enabled():
...
def admin_contractor_edit_save(cid):
    ...
    if not (request.form.get("tb_role_id") or "").strip():
        flash("Time Billing role is required.", "error")
    ...
    data = {
        ...
        "dbs_certificate_ref": (request.form.get("dbs_certificate_ref") or "").strip(),
...
def admin_contractor_dbs_check_now(cid):
    ...
    if not is_dbs_update_service_enabled():
        flash(
            "DBS Update Service checks are not enabled on this server (see DBS_UPDATE_SERVICE_ENABLED).",
            "warning",
        )
        return redirect(url_for("internal_hr.admin_contractor_profile", cid=cid))
