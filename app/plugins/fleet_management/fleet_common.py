...
def user_matches_fleet_vdi_portal_rules(user_role: str | None) -> bool:
    """
    Same VDI visibility rules as the employee portal, keyed on Sparrow ``users.role``
    (lowercased). Empty VDI selection in Fleet → Portal form access = all staff with fleet access.
    """
    ...
    rl = str(user_role or "").strip().lower()
    if rl in ("admin", "superuser", "support_break_glass"):
        return True
    vis = get_fleet_service().get_fleet_portal_form_visibility()
    vr = vis.get("vdi_roles") or []
    vc = vis.get("vdi_categories") or []
    if not fleet_portal_visibility_slot_configured(vr, vc):
        return True
    names = {rl} - {""}
    if not names:
        return False
    return fleet_role_names_match_portal_rules(
        names, vr, vc, fleet_user_role_category_definitions()
    )
