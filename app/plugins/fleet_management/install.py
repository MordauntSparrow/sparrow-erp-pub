...
def _migrate_fleet_extensions(conn) -> None:
    ...
    _migrate_fleet_portal_form_visibility(conn)
    _migrate_fleet_types_safety_servicing(conn)


def _migrate_fleet_portal_form_visibility(conn) -> None:
    """Who may open VDI vs portal safety checks (contractor roles + manifest category groups)."""
    import json
    ...
    try:
        cur.execute(
            """
            CREATE TABLE IF NOT EXISTS fleet_portal_form_visibility (
                id TINYINT NOT NULL PRIMARY KEY DEFAULT 1,
                vdi_roles_json JSON NOT NULL,
                safety_roles_json JSON NOT NULL,
                updated_at TIMESTAMP NOT NULL DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP
            ) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4
            """
        )
        conn.commit()
        cur.execute("SELECT COUNT(*) FROM fleet_portal_form_visibility WHERE id = 1")
        n = cur.fetchone()[0]
        if not n:
            empty = json.dumps({"roles": [], "categories": []})
            cur.execute(
                """
            ...
            conn.commit()
            print("[fleet_management] Seeded fleet_portal_form_visibility")
    except Exception as e:
        if "Duplicate" not in str(e):
            print(f"[fleet_management] fleet_portal_form_visibility migrate: {e}")
    finally:
        try:
            cur.close()
        except Exception:
