...
from .fleet_portal_visibility import (
    fleet_form_visibility_category_rows,
)
from .objects import (
    FLEET_ISSUE_STAGES,
...
@fleet_bp.route("/portal-form-access", methods=["GET", "POST"])
@login_required
@fleet_access_required
@fleet_edit_required
def fleet_portal_form_access():
    """Who may open VDI vs workshop safety checks (employee portal + /VDIs)."""
    svc = get_fleet_service()
    if request.method == "POST":
        try:
            svc.set_fleet_portal_form_visibility(
                vdi_roles=request.form.getlist("vdi_roles"),
                vdi_categories=request.form.getlist("vdi_categories"),
                safety_roles=request.form.getlist("safety_roles"),
                safety_categories=request.form.getlist("safety_categories"),
                user_id=_uid(),
            )
            flash("Portal form access saved.", "success")
        except Exception as e:
            logger.exception("fleet_portal_form_access")
            flash(str(e), "danger")
        return redirect(url_for("fleet_management.fleet_portal_form_access"))
    vis = svc.get_fleet_portal_form_visibility()
    role_choices = [
        (n.strip().lower(), n)
        for n in list_time_billing_role_names_for_picklist()
        if (n or "").strip()
    ]
    return render_template(
        "admin/fleet_portal_form_access.html",
        form_role_visibility=vis,
        form_visibility_category_rows=fleet_form_visibility_category_rows(),
        form_visibility_role_choices=role_choices,
    )
