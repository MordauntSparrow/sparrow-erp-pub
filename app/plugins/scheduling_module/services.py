...
class ScheduleService:
    ...
    @staticmethod
    def _get_contractor_profile_skills(cur, contractor_id: int) -> Tuple[List[str], List[str]]:
        """
        Read contractor skills/qualifications from ventus_response_module's mdt_crew_profiles by contractor_id.
        Returns (skills, qualifications). Empty if unavailable.
        """
        try:
            ...
            try:
                ...
                if r.get("qualifications_json"):
                    quals = json.loads(r["qualifications_json"]) if isinstance(r["qualifications_json"], str) else (r["qualifications_json"] or [])
            except Exception:
                skills, quals = [], []
            return (
                [str(x).strip().lower() for x in (skills or []) if str(x).strip()],
                [str(x).strip().lower() for x in (quals or []) if str(x).strip()],
            )
        except Exception:
            return [], []
    @staticmethod
    ...
    def check_open_shift_eligibility(contractor_id: int, shift: Dict[str, Any]) -> Tuple[bool, str]:
        ...
        try:
            ...
            if req.get("skills") or req.get("qualifications"):
                prof_skills, prof_quals = ScheduleService._get_contractor_profile_skills(cur, contractor_id)
                missing_skills = [r for r in req.get("skills", []) if (r or "").strip().lower() not in prof_skills]
                missing_quals = [r for r in req.get("qualifications", []) if (r or "").strip().lower() not in prof_quals]
    ...
    def create_open_shift_claim(shift_id: int, contractor_id: int) -> Tuple[bool, str]:
        ...
        try:
            ...
            if req.get("skills") or req.get("qualifications"):
                prof_skills, prof_quals = ScheduleService._get_contractor_profile_skills(cur, contractor_id)
                missing_skills = [r for r in req.get("skills", []) if r.strip().lower() not in prof_skills]
                missing_quals = [r for r in req.get("qualifications", []) if r.strip().lower() not in prof_quals]
                if missing_skills or missing_quals:
                    parts = []
                    if missing_skills:
                        parts.append("skills: " + ", ".join(missing_skills))
