...
def api_get_contractor(user_id):
    ...
    try:
        ...
        row["primary_role_label"] = None
        row["wage_card_label"] = None
        if row.get("role_id") is not None:
            try:
                cur.execute(
                    "SELECT name FROM roles WHERE id = %s LIMIT 1",
                    (int(row["role_id"]),),
                )
                rlab = cur.fetchone()
                if rlab:
                    row["primary_role_label"] = rlab.get("name")
            except Exception:
                pass
        if row.get("wage_rate_card_id") is not None:
            try:
                cur.execute(
                    "SELECT name FROM wage_rate_cards WHERE id = %s LIMIT 1",
                    (int(row["wage_rate_card_id"]),),
                )
                wlab = cur.fetchone()
                if wlab:
                    row["wage_card_label"] = wlab.get("name")
            except Exception:
                pass
...
def api_update_contractor(user_id):
    ...
    invoice_billing_frequency = (
        data.get("invoice_billing_frequency") or "weekly"
    ).strip().lower()
    if invoice_billing_frequency not in ("weekly", "monthly"):
    ...
    try:
        ...
        # update base fields (employment_type / invoice billing frequency if columns exist)
        try:
            cur.execute("""
                UPDATE tb_contractors
                SET name = %s, status = %s, employment_type = %s,
                    invoice_billing_frequency = %s
                WHERE id = %s
            """, (name, status, employment_type, invoice_billing_frequency, user_id))
        except (mysql.connector.Error, Exception):
            try:
                cur.execute("""
                    UPDATE tb_contractors
        ...
        try:
            ...
            sync_ventus_crew_profile_from_hr_training(int(user_id))
        except Exception:
            pass
