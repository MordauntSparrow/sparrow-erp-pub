...
def admin_search_visits_picker(q: str, limit: int = 20) -> List[Dict[str, Any]]:
    """Autocomplete for work visit id (photo gallery filter)."""
    if not _work_visits_table_exists():
        return []
    qs = (q or "").strip()
    if len(qs) < 2:
    ...
    try:
        ...
        if vid is not None:
            cur.execute(
                """
                SELECT v.id AS visit_id, v.work_date, c.name AS client_name,
                       u.name AS contractor_name, v.schedule_shift_id
                FROM work_visits v
                JOIN clients c ON c.id = v.client_id
                JOIN tb_contractors u ON u.id = v.contractor_id
                WHERE v.id = %s OR c.name LIKE %s OR u.name LIKE %s
                ORDER BY v.work_date DESC, v.id DESC
                LIMIT %s
                """,
                (vid, like, like, lim),
            ...
            cur.execute(
                ...
                JOIN clients c ON c.id = v.client_id
                JOIN tb_contractors u ON u.id = v.contractor_id
                WHERE c.name LIKE %s OR u.name LIKE %s OR CAST(v.id AS CHAR) LIKE %s
                ORDER BY v.work_date DESC, v.id DESC
                LIMIT %s
                """,
                (like, like, like, lim),
        ...
        for r in cur.fetchall() or []:
            ...
            sid = r.get("schedule_shift_id")
            out.append(
                {
                    "id": iid,
                    "label": f"Visit #{iid} · {day} · {cname} · {cn} · shift {sid}",
                }
            )
        return out
    except Exception as e:
        logger.warning("admin_search_visits_picker: %s", e)
        return []
    finally:
        cur.close()
        conn.close()
...
def admin_visit_filter_label(visit_id: int) -> Optional[str]:
    if not _work_visits_table_exists():
        return None
    conn = get_db_connection()
    cur = conn.cursor(dictionary=True)
    try:
        ...
        cn = (r.get("contractor_name") or "").strip() or "—"
        sid = r.get("schedule_shift_id")
        return f"Visit #{iid} · {day} · {cname} · {cn} · shift {sid}"
    except Exception:
        return f"Visit #{int(visit_id)}"
    finally:
        cur.close()
...
def update_shift_times_admin(
    ...
    _get_schedule_service().update_shift(shift_id, updates, portal_notify=False)
...
def _sync_shift_to_time_billing_impl(shift_id: int) -> None:
    """Delegates to Time Billing (no work_module dependency for callers such as scheduling)."""
