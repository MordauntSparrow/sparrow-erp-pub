...
@internal_bp.get("/photos/visits-search")
@login_required
@_admin_required_work
def admin_visits_search():
    q = (request.args.get("q") or "").strip()
    if len(q) < 2:
        return jsonify([])
    return jsonify(work_services.admin_search_visits_picker(q, limit=20))
...
def admin_photos():
    ...
    contractors = _get_contractors()
    visit_filter_label = (
        work_services.admin_visit_filter_label(int(visit_id)) if visit_id else None
    )
    return render_template(
        "admin/photos.html",
        photos=photos,
        ...
        shift_id=shift_id,
        visit_id=visit_id,
        visit_filter_label=visit_filter_label,
        date_from=date_from,
        date_to=date_to,
        config=_core_manifest,
    )
